...
path_m_eg = "../Seg/label/"
path_binary = "../EEC/EEC_save_sampling_information/" + n + "/binary/"
...
class Label:
    ...
    def update_(self, img, sl, ed, p, r, c):
        ...
        # 生成填充掩膜
        filled_mask = np.zeros((self.sp[0], self.sp[1]), np.uint8)
        cv2.drawContours(filled_mask, contours, -1, 255, -1)

        return contours, filled_mask
    ...
    def run(self):
        ...
        while True:
            # 检查窗口是否仍然存在
            try:
                window_status = cv2.getWindowProperty("image", cv2.WND_PROP_VISIBLE)
                if window_status < 1:  # 窗口被关闭
                    break
            except:
                break  # 如果窗口已经被销毁
        ...
        if input("input:") == "y":
            ...
            if not (os.path.exists(save_sample_path + "/sample_images")):
                os.makedirs(save_sample_path + "/sample_images", exist_ok=True)
            cv2.imwrite(
            ...
            # 生成最终二值图
            final_mask = np.zeros((self.sp[0], self.sp[1]), np.uint8)
            for center in self.cent:
                _, single_mask = self.update_(self.img2.copy(), self.slic,
                                              np.zeros((self.sp[0], self.sp[1]), np.uint8),
                                              center, self.r_min, [])
                final_mask = cv2.bitwise_or(final_mask, single_mask)

            cv2.imwrite(f"{save_sample_path}{n}/final_binary_mask.png", final_mask)
        else:
...
if __name__ == "__main__":
    ...
    if not (os.path.exists(save_sample_path + n + "/nms")):
        os.makedirs(save_sample_path + n + "/nms", exist_ok=True)
    if not (os.path.exists(save_sample_path + n + "/source")):
        os.makedirs(save_sample_path + n + "/source", exist_ok=True)
